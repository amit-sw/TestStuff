...
def one_run(content):
    ...
    with st.sidebar.expander("Token Details - base file"):
...
def main():
    ...
    tab1,tab2=st.tabs(['File Upload',"Direct Text"])
    with tab1:
        ...
        if file_input is not None:
            ...
            one_run(content)
    with tab2:
        text_content=st.text_area("Input transcript directly")
        if st.button("Analyze"):
            one_run(text_content)
